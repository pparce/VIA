...
public class BuscarSenalesFragment extends Fragment implements CallbackFragmentBuscarSenales {
    ...
    private void Inicializar() {
        ...
        for (int i = 0; i < App.LISTA_SENALES.size(); i++) {
            for (int j = 0; j < App.LISTA_SENALES.get(i).getListaTipoSenales().size(); j++) {
                List<SenalModelo> listaAux = App.LISTA_SENALES.get(i).getListaTipoSenales().get(j).getListaSenal();
    ...
    private List<SenalModelo> filter(List<SenalModelo> articulos, String texto) {

        marcado = texto;
        List<SenalModelo> list = new ArrayList<>();
        try {
            texto = texto.toLowerCase().trim();
            for (SenalModelo senal : articulos) {
    ...
    public class BuscarAsyncSenal extends AsyncTask<Void, Void, List<SenalModelo>> {

        String texto;
        List<SenalModelo> listaArticulos;
        CallBackBuscar callBackBuscar;

        public BuscarAsyncSenal(CallBackBuscar callBackBuscar, String texto, List<SenalModelo> listaArticulos) {
        ...
        @Override
        protected List<SenalModelo> doInBackground(Void... voids) {

            List<SenalModelo> listafiltrada = filter(listaArticulos, texto);
            return listafiltrada;
        }

        @Override
        protected void onPostExecute(List<SenalModelo> modeloArticulos) {
