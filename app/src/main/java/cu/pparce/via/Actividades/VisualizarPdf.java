...
import cu.pparce.via.Utiles.CallBacks.CallbackLibro;

import cu.pparce.via.Aplicacion;
import cu.pparce.via.R;
import cu.pparce.via.Utiles.CallBacks.CallbackArbolArticulos;
import cu.pparce.via.Utiles.PdfViewerView;
import cu.pparce.via.Utiles.recyclertreeview_lib.TreeNode;
import cu.pparce.via.Utiles.recyclertreeview_lib.TreeViewAdapter;
import cu.pparce.via.Utiles.recyclertreeview_lib.app.Dir;
import cu.pparce.via.Utiles.recyclertreeview_lib.app.DirectoryNodeBinder;
import cu.pparce.via.Utiles.recyclertreeview_lib.app.File;
import cu.pparce.via.Utiles.recyclertreeview_lib.app.FileNodeBinder;

public class VisualizarPdf extends AppCompatActivity implements CallbackLibro, CallbackArbolArticulos {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        posicicion = bundle.getInt("posicion");

        switch (posicicion) {
            case 0:

                initTree();
                setTheme(R.style.AppTheme_NoActionBarVisualizar1);

                break;
            case 1:

                Inicializar("Codigo vial ilustrado.pdf");


                break;
            case 2:

                Inicializar("Guia de estudio.pdf");
    ...
    private void initData() {
        ...
        Aplicacion.callbackArbolArticulo = this;
    ...
    /*private TreeNode<File> addArticulo(int first, int end) {
    ...
    private void Inicializar(final String libro) {

        Aplicacion.callbackLibro = this;

        setContentView(R.layout.activity_visualizar_pdf);
        pdfViewerView = (PdfViewerView) findViewById(R.id.pdfViewr);
        //SWIPE VERTICAL
                /*pdfViewerView.setPageTransformer(true, new VerticalPageTrasnformer());
                pdfViewerView.setOverScrollMode(View.OVER_SCROLL_NEVER);*/
        pdfViewerView.setPdfFromAsset(libro);

        SharedPreferences preferences = getSharedPreferences("posiciones_libros", Context.MODE_PRIVATE);
        pdfViewerView.setCurrentItem(preferences.getInt(libro, 0));
        toolbarmenu = (Toolbar) findViewById(R.id.toolbar_menu);
        setSupportActionBar(toolbarmenu);
        getSupportActionBar().setTitle(libro.replaceAll(".pdf", ""));
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        menu = (LinearLayout) findViewById(R.id.menu);
        menu = (LinearLayout) findViewById(R.id.menu);


        pdfViewerView.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            @Override
            ...
            public void onPageSelected(int position) {
                hideMenu();
                seekBarPagina.setProgress(position);

                SharedPreferences preferences = getSharedPreferences("posiciones_libros", Context.MODE_PRIVATE);
                SharedPreferences.Editor editor = preferences.edit();
                editor.putInt(libro, position);
                editor.commit();
            }
        ...
        seekBarPagina = (SeekBar) findViewById(R.id.seekBarPagina);
        indicadorPagina = (TextView) findViewById(R.id.indicadorPagina);
        seekBarPagina.setMax(pdfViewerView.getAdapter().getCount());
        seekBarPagina.setProgress(pdfViewerView.getCurrentItem() + 1);
        indicadorPagina.setText(pdfViewerView.getCurrentItem() + 1 + "/" + (pdfViewerView.getAdapter().getCount() + 1));
        seekBarPagina.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override
            public void onProgressChanged(SeekBar seekBar, int i, boolean b) {
                indicadorPagina.setText((i + 1) + "/" + (pdfViewerView.getAdapter().getCount() + 1));
            }
            ...
            public void onStopTrackingTouch(SeekBar seekBar) {
                pdfViewerView.setCurrentItem(seekBar.getProgress());
            }
        });
    ...
    private void showMenu() {
        toolbarmenu.animate().translationY(0).setDuration(300).start();
        menu.animate().translationY(0).setDuration(300).start();
        flag = true;
    }

    private void hideMenu() {
        toolbarmenu.animate().translationY(-toolbarmenu.getHeight()).setDuration(300).start();
    ...
    public void onClick(String nombre, int posicion) {
        ...
        if (nombre.equals("DISPOSICIONES ESPECIALES")) {
            ...
            setDialog(nombre, Aplicacion.LISTA_REGULACIONES.get(0));

        } else if (nombre.equals("DISPOSICIONES TRANSITORIAS")) {

            setDialog(nombre, Aplicacion.LISTA_REGULACIONES.get(1));


        } else if (nombre.equals("REGULACIONES FINALES")) {

            setDialog(nombre, Aplicacion.LISTA_REGULACIONES.get(2));
            ...
            if (textView.getText().equals("REGULACIONES COMPLEMENTARIAS")) {
                ...
                setDialog(nombre, Aplicacion.LISTA_ARTICULOS_REGULACIONES.get(pos - 1).getDescripcion());

            } else {
                String[] aux = nombre.split(" ");
                int pos = Integer.parseInt(aux[1]);

                setDialog(nombre, Aplicacion.LISTA_ARTICULOS_LIBROS.get(pos - 1).getDescripcion());
    ...
    private void setDialog(String title, String message) {
        ...
        final AlertDialog.Builder builder = new AlertDialog.Builder(VisualizarPdf.this);
        builder
                .setTitle(title)
                .setView(myview)
