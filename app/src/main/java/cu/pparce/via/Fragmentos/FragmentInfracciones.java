...
import cu.pparce.via.Actividades.Buscar;
import cu.pparce.via.Adaptadores.ViewPagerBuscarAdapter;
import cu.pparce.via.Adaptadores.ViewPagerInfraccionesAdapter;
...
public class FragmentInfracciones extends Fragment implements CallBackFragmentInfraccion, SearchView.OnQueryTextListener  {
    ...
    private void initView() {
        ...
        Aplicacion.callBackFragmentInfraccion = this;
        contenedorBuscar = (LinearLayout) view.findViewById(R.id.contenedor_buscador);

        //TABLAYOUT Y VIEW PAGER
        tab = (TabLayout) view.findViewById(R.id.tab);

        for (int i = 0; i < Aplicacion.LISTA_INFRACCION.size(); i++) {
            tab.addTab(tab.newTab().setText(Aplicacion.LISTA_INFRACCION.get(i).getTipo()));
        }
    ...
    public boolean onQueryTextChange(String newText) {
        ...
        for (int i = 0; i < Aplicacion.listaCallBack.size(); i++) {
            Aplicacion.listaCallBack.get(i).setBusquedaArticulos(newText);
        }
        return true;
    }
